...
export default class SchemaManager {
    ...
    fileIO: IFileIO;
    converters: FileToSchemaConverter[];
    ...
    constructor(
        ...
        this.fileIO = fileIO;
        this.converters = fileToSchemaConverters;
    }

    /**
     * Main function that runs the script, parses the schema files and applies them to .liquid files
     * @param { string } schemas_path           - Path to schema files directory. 
     * @param { string } liquid_sections_path   - Path to liquid sections directory.
     * @returns { boolean } True on success, false on failure.
     */
    async run( schemas_path: string, liquid_sections_path: string): Promise<boolean> {

        validateType( schemas_path, 'schemas_path', 'string' );
        validateType( liquid_sections_path, 'liquid_path', 'string');
        ...
        this.applySchemasToLiquid( resolved_schemas, liquid_sections_path );
     ...
     */
    getSchemaFiles ( schemas_dir_path: string ): string[] {
     ...
     */
     applySchemasToLiquid( resolved_schemas: ObjSchema[], liquid_sections_path: string): boolean {
        ...
        let did_update_any_files: boolean = false;
        ...
        for ( const schema of schemas ) {
            ...
            if( all_liquid_files.includes( target_file ) ) {
                ...
                const did_update_file:boolean = this.applySchemaToLiquidFile(schema, liquid_sections_path, target_file);
                if ( did_update_file ) {
                    did_update_any_files = true;
                }
            }

        }

        if( ! did_update_any_files ) {
            console.log( `Didn't modify any files` );
        }
     ...
     * 
     * @return { boolean } - True if file was modified, false if not. Throws error on failure.
     */
    applySchemaToLiquidFile( schema: ObjSchema, liquid_sections_path: string, target_file: string, mode: WriteModeEnum = WriteModeEnum.OverwriteAll): boolean {
        ...
        if ( mode == WriteModeEnum.OverwriteAll ) {
            ...
            did_update_file = this.fileIO.replaceInFile(/{%\s*schema\s*%}.*{%\s*endschema\s*%}/gs, schema_output, full_path);
     ...
     */
    getFunctionFromFnSchema(all_schemas: SchemasList, schema_name: string): Function | null {
     ...
     */
    getObjFromObjSchema(all_schemas: SchemasList, schema_name: string): GenericObject | null {
     ...
     */
    parseSchema(all_schemas: SchemasList, schema_name: string, schema_args: any[]): GenericObject {
     ...
     */
    resolveSchemas( schemas_list: SchemasList ): ObjSchema[] {
     ...
     */
    async schemaFilesToSchemasList( schemas_path: string, filenames: string[]): Promise<SchemasList> {
     ...
     */
    async schemaFileToObj( file_path: string): Promise<Schema> {
        ...
        const converter = this.converters.find( (converter: FileToSchemaConverter) => converter.file_extension == file_ext);
